Item1 = input("Please enter your first item:")
Price1 = input("Please enter the price of your first item:")
Item2 = input("Please enter your second item:")
Price2 = input("Please enter the price of your second item:")
Item3 = input("Please enter your third item:")
Price3 = input("Please enter the price of you third item:")
Item4 = input("Please enter your fourth item:")
Price4 = input("Please enter the price of your fourth item:")

Subtotal = (Price1 + Price2 + Price3 + Price4)

print("<<<<<<<<<<Recipt>>>>>>>>>>")
















